<template>
  <div class="session">
    ...
    <el-input placeholder="请输入 url" v-model="url">
      <template #append>
        <el-button type="primary" icon="el-icon-search" @click="sendRequest">发送</el-button>
      </template>
    </el-input>
    <p>返回值 {{ response }}</p>
    ...
    <el-input placeholder="请输入域名" v-model="host">
      <template #append>
        <el-button icon="el-icon-search" @click="resolveHost">发送</el-button>
      </template>
    </el-input>
    <p>返回值 {{ resolvedIP }}</p>
  </div>
...
<script>
import { getGlobal, session } from '@electron/remote';
