...
if (!gotTheLock) {
  ...
  app.on('ready', () => {
    ...
    /************************** 接收渲染进程的消息 ****************************/
    // 同步消息
    ipcMain.on('sync-render', (event, args) => {
      event.returnValue = '我收到了同步消息，这是我的回复：啦啦啦';
    });

    // 异步消息（send）
    ipcMain.on('async-render', (event, args) => {
    ...
    // 监听从渲染进程发送的 Notification 请求
    ipcMain.on('show-notification', (event, args) => {
      showNotification();
    });

    // 监听从渲染进程发送的更新 Dock 图标数字的请求
    ipcMain.on('update-dock-badge', (event, badgeCount) => {
      // 设置 Dock 图标的数字
